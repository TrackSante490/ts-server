...
from sensor_public_model import (  # noqa: E402
    ...
    download_occupancy_detection_dataset,
    train_occupancy_public_model,
)
import train_bidmc_public_model as common  # noqa: E402


LOGGER = logging.getLogger("occupancy_public_model.train")
common.LOGGER = LOGGER


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Train a PyTorch public environment-context anomaly model from the UCI Occupancy Detection dataset."
    )
    parser.add_argument(
        "--dataset-dir",
        help="Path to an extracted Occupancy Detection dataset directory containing CSV/TXT files for ambient temperature/humidity/CO2 training.",
    )
    parser.add_argument(
        "--download-dir",
        help="Directory to download and extract the Occupancy Detection dataset into. If omitted, --dataset-dir is required.",
    )
    parser.add_argument(
        "--output",
        default=str(REPO_ROOT / "api" / "models" / "occupancy_public_model.pt"),
        help="Path to write the trained PyTorch artifact.",
    )
    parser.add_argument("--window-seconds", type=int, default=1800)
    parser.add_argument("--step-seconds", type=int, default=600)
    ...
    if not dataset_dir:
        ...
        print(f"Downloading Occupancy Detection dataset from {OCCUPANCY_DETECTION_DATASET_URL} ...")
        dataset_dir = str(download_occupancy_detection_dataset(args.download_dir))
    ...
    with progress.redirect_context:
        artifact = train_occupancy_public_model(
